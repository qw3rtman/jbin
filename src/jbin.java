...
public class jbin
{
	public static void jarToBinary(String jar, String binary) throws IOException
	{
		...
		try {
			...
			InputStream shebangInputStream = new ByteArrayInputStream("#!/usr/bin/java -jar\n".getBytes("UTF-8"));
	...
	public static void sourceToJAR(String main, String[] additional, String jar) throws IOException, InterruptedException
	{
		...
		try {
			...
			if (!manifest.exists()) { // If it doesn't exist...
				...
				String generatedByJbinComment = "# Generated by jbin (https://github.com/qw3rtman/jbin).";
				String manifestContent = "Main-Class: " + main.substring(0, main.length() - 5) + "\n";

				manifestOutput.write(manifestContent.getBytes());
				manifestOutput.write(generatedByJbinComment.getBytes());
	...
	public static void main(String[] args) throws IOException, InterruptedException {
		...
		String[] additional;
		if (args.length > 1) {
			additional = Arrays.copyOfRange(args, 1, args.length);
		} else {
			additional = new String[0];
		}
		String bin = main.substring(0, main.length() - 5);
		String jar = bin + ".jar";
